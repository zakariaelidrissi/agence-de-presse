        ...
        <div class="card shadow mb-4">
            @foreach ($articles as $article)                
                @if ($article->id == $article_id)
                    <form action="{{ route('red.update.article', $article->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control @error('title') is-invalid @enderror" placeholder="Ajouter tite d'article" id="titrArticle" name="title" value="{{ $article->title }}">
                                    @error('title')
